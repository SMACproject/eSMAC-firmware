...
int spi_ch = SPI_CH_0;
int spi_speed = SPI_SPEED_1000000;
...
void MPL115A1_write_register(unsigned char address, unsigned char data)
{
	...
	if((flash = MPSSE(SPI0, CS, TEN_MHZ, MSB)) != NULL && flash->open)
	{
...
unsigned char MPL115A1_read_register(unsigned char address)
{
	...
	if((flash = MPSSE(SPI0, CS, TEN_MHZ, MSB)) != NULL && flash->open)
	{
		...
		Start(flash);
		spiRead = Transfer(flash, spiData, 2);
		data = spiRead[1];
		Stop(flash);
	}
	else
...
float MPL115A1_read_pressure(void)
{
    ...
    usleep(3000); // AN: data is typically ready after 3ms, DS for both: max. 1ms
...
float MPL115A1_read_temperature(void)
{
    ...
    printf("*** MPL115A1_read_temperature: start temp\n");
    MPL115A1_write_register(MPL115A1_STARTTEMP, 0x00); // start temperature conversion
    usleep(3000); // AN: data is typically ready after 3ms, DS for temp: max. 0.7ms

    // read raw temperature
    printf("*** MPL115A1_read_temperature: TEMPH\n");
    uiTH = MPL115A1_read_register(MPL115A1_TEMPH);
    printf("*** MPL115A1_read_temperature: TEMPL\n");
    uiTL = MPL115A1_read_register(MPL115A1_TEMPL);
...
int main(int argc, char *argv[])
{
    ...
    double power = pow(10, (log10( (double)(pressure*10/times) / 1013.25) / 5.2558797)) - 1;
    double altitude = power / (-6.8755856 * pow(10, -6));
    printf("Altitude: %lf %.4lf meters\n\n", power, altitude/10);
